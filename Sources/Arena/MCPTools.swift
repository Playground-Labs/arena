...
enum MCPTools {
    ...
    Review is read-only until status is consensus (author acceptance or unanimous agreement). Do not modify project files or external systems before consensus, even when implementation was requested. The implementing agent privately saves proposed changes, affected files, rationale, and remaining objections in a session-specific file under its local temporary directory, then records the exact final assessment and verifies current consensus before acting within existing user authorization. Impasse and Stopped do not authorize implementation. Keep comments and rebuttals concise but complete: decision, evidence, trade-offs, next step. Omit greetings, praise, filler, and repeated agreement; use Caveman-style compression when clarity survives. Proposals are different: a human reads and accepts them. Write the propose_outcome assessment in plain, complete sentences with minimal jargon, leading with the recommendation, then why, trade-offs, and remaining risks. Format for readability: short paragraphs, hyphen bullets, and bold lead-ins only; no headings, tables, or code fences.
    """
