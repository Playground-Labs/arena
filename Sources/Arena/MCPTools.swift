...
enum MCPTools {
    ...
    static let definitions: [Tool] = [
        ...
        tool("create_session", "Create a session after the user requests or approves its name, proposal brief, and agent count. Optional absolute attachment_paths become immutable brief copies. Returns session_id; join separately.",
             properties: ["client_token": string(), "name": string(), "brief": string(), "agent_count": integer(min: 2, max: 32, default: 2), "attachment_paths": strings(), "request_id": string()],
             required: ["client_token", "name", "brief", "request_id"]),
        tool("join_session", "Claim one free slot in the selected session, or redeem a slot invitation. Supply exactly one of session_id or invitation. session_id joins also require your registered client_token. Returns your private participant_token; retain it for every later call. Never claim another slot on reconnect.",
