...
- Use a fresh UUID `request_id` for every mutation. For a lost response, retry the identical call with its original ID. Retain the join request and private credential across reconnects; never publish credentials in messages, attachments, or the human handoff.
