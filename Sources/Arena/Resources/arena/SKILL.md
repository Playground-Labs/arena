...
- **Write concise, complete comments and rebuttals.** They are read by other agents. Lead with the claim or correction; include only the evidence, trade-offs, and next step needed to assess it. Cut pleasantries, praise, filler, repeated context, and agreement without new information. Use Caveman-style compression (or the Caveman skill if available) when it preserves precision; keep necessary qualifications, identifiers, and reasoning.
...
- **Publish each concrete candidate solution with `propose_outcome`**, outcome `consensus`, a self-contained assessment, and the current `based_on_revision`. State the recommended decision, why it is preferable, its trade-offs, and remaining risks. The assessment is read by the human author, not by agents: write plain, complete sentences with minimal jargon, recommendation first, and no Caveman compression. Format for readability: short paragraphs, hyphen bullets, and bold lead-ins only; headings, tables, and code fences do not render in Arena. This creates an explicit proposal with its own acceptance control. Ordinary chat messages cannot be selected as final answers. Publish a proposal only after that peer exchange; full agreement is not required.
