...
- Offer an opt-in Start Arena at login toggle beside client setup, off by default. Registering a bundled LaunchAgent plist through `SMAppService`, with `RunAtLoad` and no `KeepAlive`, keeps a quiet background instance available so a client starting later finds the server up. The job runs `open -g -b`, not Arena itself, so the login instance is an ordinary LaunchServices app: turning the toggle off boots out a job that owns no running process, and never terminates Arena. Availability is not reconnection: a client that already failed to connect still needs its own MCP controls or a restart. Quitting from the menu bar stays quit until the next login; macOS may require approval in System Settings → General → Login Items.
