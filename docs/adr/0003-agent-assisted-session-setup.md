...
Setup retries are scoped to a server-issued persistent client credential. Agents register once before creation or a session-ID join and privately retain the token across transport reconnects. Two agents using the same model label and request ID therefore receive separate slots and receipts. Invitation joins retain their invitation-based retry scope. Existing participant credentials remain valid; legacy global setup receipts cannot be replayed by newly registered callers.
